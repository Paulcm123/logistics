...
<div class="card">
	<div class="card-header">
		<h1 class="card-title">AWAITING PICKUP</h1>
	</div>
	<div class="card-body">
		<div class="table-responsive">
			<table class="table table-striped table-hover">
				...
				<tbody>
					...
					$shipments=$conn->query("select distinct shipments.id,shipments.date,shipments.status,sender.username sender,receiver.username receiver,origin.name origin,destination.name destination from shipments inner join users on users.location=shipments.origin inner join users sender on shipments.sender_id=sender.id inner join users receiver on shipments.receiver_id=receiver.id inner join locations origin on shipments.origin=origin.id inner join locations destination on shipments.destination=destination.id where shipments.origin=".$_SESSION['user']['location']." and shipments.status='Awaiting pickup'");
